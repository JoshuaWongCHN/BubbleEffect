...
void main() {
    ...
    vec3 hemiLightDir = normalize(position);
    float ratio = (dot(hemiLightDir, vec3(0.0, 1.0, 0.0)) + 1.0) / 2.0;
    vec3 color = mix(u_groundColor, u_skyColor, ratio);
    ratio = clamp(dot(normal, hemiLightDir), 0.0, 1.0);
    v_lightDiffuse += color * ratio;
}
